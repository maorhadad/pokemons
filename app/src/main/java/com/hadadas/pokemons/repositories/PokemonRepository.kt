...
class PokemonRepository(private val database: IPokemonDb, private val pokemonApi: IPokemonApi) : IPokemonRepository {

    override suspend fun fetchPokemons(limit: Int, offset: Int): List<PokemonShort> {
        var pokemons = database.getPokemons(limit, offset)
        if (pokemons.isEmpty()) {
    ...
    override suspend fun fetchPokemonsDetails(pokemonName: String, pokemon: MutableLiveData<Pokemon>) {
        withContext(Dispatchers.IO) {
            ...
            database.insertPokemonInfo(pokemonDetails)
